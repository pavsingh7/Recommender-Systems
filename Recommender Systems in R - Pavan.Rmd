...
subtitle: An introduction to recommender systems in R
author: 
- name: "Pavan Singh"
...
rm(list=ls())
setwd("~/sngpav003@myuct.ac.za - Google Drive/My Drive/Masters 2022/Data Science for Industry/Lecture 2 Recommender Systems")
```
...
This approach has an intuitive appeal but is pretty unsophisticated (everyone gets the same recommendations, barring the filtering out of seen movies!) In other words, everyone's vote counts the same. Collaborative filtering overcomes these inadequacies. Let's look at the three approaches below. 

## User-Based (*between users*)

This type of CF is a way of taking a user’s interests into account by looking for users who are somehow similar to them, and then suggest the things that those users are interested in. In order to do that, we’ll need a way to measure how similar two users are. There are lots of different similarity measures. Here we’ll use *cosine similarity*. We’ll apply this to vectors of $0$s and $1$s, each vector representing one user’s interests. It will be 1 if the user specified the $i^{th}$ interest, and $0$
otherwise. Accordingly, “similar users” will mean “users whose interest vectors most nearly point in the same direction”. 
